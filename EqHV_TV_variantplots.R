###### Experimental cohort variant plots #####
#
...
##### Figure: Variant plot TV ####


# filter for experimentally infection data only and set levels as facet labels
...
dummy <- iSNV_eq+
  ...
  scale_fill_manual(values = TV_colors)+
...
entropy_df_indiv_eqHV <- entropy_df_indiv %>%
  ...
  geom_line()+
  scale_color_manual(values = TV_colors, label=c("Horse F", "Horse G", "Horse H", "Horse I", "InoculumII", "Inoculum I"), name="Animals")+
...
dnds_df_stats_richnessSNV <- dnds_entropy_df %>%
  filter(TVid %in% c("horseF", "horseG", "horseH", "horseI")) %>%
