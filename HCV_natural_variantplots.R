...
##### Figure: Variant plot HCV ####


# filter for HCV infection data only and set levels as facet labels
...
dummy <- iSNV_eq+
  ...
  scale_fill_manual(values = HCV_colors)+
...
entropy_df_indiv_eqHV <- entropy_df_indiv %>%
  ...
  geom_line()+
  scale_color_manual(values = HCV_colors, label=c("Patient J", "Patient K", "Patient L"), name="Patients")+
...
dnds_df_stats_richnessSNV <- dnds_entropy_df %>%
  filter(TVid %in% c("patientJ", "patientK", "patientL")) %>%
